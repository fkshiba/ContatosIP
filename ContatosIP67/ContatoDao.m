...
-(void) insereContatoDefault {
    NSUserDefaults *config = [NSUserDefaults standardUserDefaults];
    BOOL inseriu = [config boolForKey:@"inseriu"];
    if (!inseriu) {
        ...
        contato.nome = @"Felipe";
        contato.telefone = @"1932567468";
        contato.email = @"felipe.shiba@gmail.com";
        contato.site = @"http://www.felipeshiba.com";
        contato.endereco = @"São Paulo, Rua Vergueiro, 3185";
        contato.latitude = [NSNumber numberWithDouble:-23.5883034];
        contato.longitude = [NSNumber numberWithDouble:-46.632369];
    
        [self saveContext];
        [config setBool:YES forKey:@"inseriu"];
        [config synchronize];
    }
}
...
-(void) listaContatos {
    ...
    NSArray* contatosQuery = [self.managedObjectContext executeFetchRequest:query error:nil];
    _contatos = [contatosQuery mutableCopy];
}
...
- (NSURL *)applicationDocumentsDirectory {
    // The directory the application uses to store the Core Data store file. This code uses a directory named "com.felipeshiba.ContatosIP67" in the application's documents directory.
